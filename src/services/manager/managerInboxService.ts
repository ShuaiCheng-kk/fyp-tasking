...
export const managerInboxService = {
  ...
  async getConversations(userId: string) {
    const messages = await managerInboxRepository.getConversationPartners(userId)
      ...
      Array.from(partnerMap.values()).map(async (conv) => {
        ...
        try {
          const [user, company] = await Promise.all([
            managerInboxRepository.findUserById(conv.partnerId),
            conv.companyId ? managerInboxRepository.findCompanyById(conv.companyId) : Promise.resolve(null),
          ])
          if (user) {
  ...
  async getMessages(userId: string, otherUserId: string) {
    const messages = await managerInboxRepository.getMessagesBetweenUsers(userId, otherUserId)
    await managerInboxRepository.markMessagesAsRead(userId, otherUserId)
    return messages
  },
  ...
  async sendMessage(fromUserId: string, toUserId: string, companyId: string, content: string) {
    ...
    try {
      const sender = await managerInboxRepository.findUserById(fromUserId)
      if (sender) senderName = (sender as any).full_name ?? undefined
    } catch {}
    return managerInboxRepository.insertMessage(fromUserId, toUserId, companyId, content.trim(), senderName)
  },

  async getNotifications(userId: string) {
    return managerInboxRepository.getNotifications(userId)
  },
  ...
  async updateNotificationStatus(notificationId: string, status: string) {
    ...
    return managerInboxRepository.updateNotificationStatus(notificationId, status)
  },
  ...
  async getUnreadCount(userId: string, companyId: string | null, lastAnnouncementReadAt?: string | null) {
    const [unreadMessages, unreadAnnouncements, pendingInvitations] = await Promise.all([
      managerInboxRepository.countUnreadMessages(userId),
      companyId ? managerInboxRepository.countUnreadAnnouncements(companyId, lastAnnouncementReadAt ?? null) : Promise.resolve(0),
      managerInboxRepository.countPendingInvitations(userId),
    ])
    return {
  ...
  async getInvitesByRecipient(user_id: string) {
    return managerInboxRepository.getInvitesByRecipient(user_id)
  },

  async getInboxItemById(inbox_id: string) {
    return managerInboxRepository.getInboxItemById(inbox_id)
  },

  async updateInboxStatus(inbox_id: string, status: string) {
    await managerInboxRepository.updateInboxStatus(inbox_id, status)
  },
