...
export function applyClockInGracePeriod(clockTime: string, shiftDate: string, shiftStartTime: string): string {
  // shift_date/shiftStartTime are UTC-naive — the explicit 'Z' keeps this comparable to
  // clockTime, which is always a real ISO UTC timestamp.
  const scheduledStart = new Date(`${shiftDate}T${shiftStartTime}Z`)
