...
function DatePickerField({ value, onChange, placeholder = 'Select date' }: {
  ...
  // yearPage 0 = most recent block: [maxY-11 … maxY]
  ...
  // "too recent" = born within the last 16 years (not old enough)
  ...
  const headerBtnStyle: React.CSSProperties = {
    ...
    borderRadius: 8, fontFamily: fB, fontWeight: 700, fontSize: '0.9375rem',
  ...
  return (
    <div style={{ position: 'relative' }}>
      <button
        ...
        style={{
          width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'space-between',
          padding: '14px 16px', border: `1.5px solid ${open ? '#F97316' : '#E5E7EB'}`,
          borderRadius: '10px', background: '#FFFFFF', cursor: 'pointer',
          fontFamily: fB, fontSize: '1rem',
          color: label ? '#1C1917' : '#9CA3AF',
          outline: 'none', boxSizing: 'border-box', transition: 'border-color 0.15s',
        }}
      >
      ...
      {open && createPortal(
        <div ref={calRef} style={{
          ...
          zIndex: 9999, padding: '16px 16px 14px', fontFamily: fB,
        }}>
                ...
                return (
                  <button key={`d-${i}`} type="button" onClick={() => selectDay(d)} disabled={fut}
                    style={{
                      ...
                      fontFamily: fB, display: 'flex', alignItems: 'center', justifyContent: 'center',
            ...
            {value && (
              <div style={{ marginTop: 10, paddingTop: 10, borderTop: '1px solid #F3F4F6', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <button type="button" onClick={() => { onChange(''); setOpen(false) }}
                  style={{ fontSize: '0.8125rem', color: '#F97316', fontWeight: 600, background: 'none', border: 'none', cursor: 'pointer', padding: 0, fontFamily: fB }}>
                  Clear
                </button>
              </div>
            )}
          ...
          {mode === 'month' && (<>
                ...
                return (
                  <button key={name} type="button" onClick={() => selectMonth(mi)} disabled={fut}
                    style={{
                      ...
                      fontFamily: fB, textAlign: 'center', transition: 'background 0.1s',
                    }}
            ...
            <div style={{ marginTop: 12, paddingTop: 10, borderTop: '1px solid #F3F4F6', display: 'flex', justifyContent: 'flex-start' }}>
              <button type="button" onClick={() => setMode('day')}
                style={{ fontSize: '0.8125rem', color: '#6B7280', fontWeight: 500, background: 'none', border: 'none', cursor: 'pointer', padding: 0, fontFamily: fB, display: 'flex', alignItems: 'center', gap: 4 }}>
                <ChevL /> Back to days
              </button>
            </div>
          </>)}
...
function AccountFields({ form, setForm, phoneError, clearPhoneError, profilePhotoUrl, onProfilePhotoChange }: {
  form: { fullName: string; email: string; password: string; confirmPassword: string; phone: string; dateOfBirth: string };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      ...
      {/* Profile Photo */}
      <div className="gs-field" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '10px' }}>
...
export default function GetStartedPage() {
  ...
  // Invited form state
  const [invitedAccount, setInvitedAccount] = useState({ fullName: '', email: '', password: '', confirmPassword: '', phone: '', dateOfBirth: '' });
  ...
  const handleOwnerRegister = async () => {
    ...
    try {
      ...
      sessionStorage.setItem('owner_date_of_birth', ownerAccount.dateOfBirth);
  ...
  const runCompanySetup = async (plan: 'Free' | 'Paid', clearSession = true) => {
    ...
    const res = await fetch('/api/owner/complete-company-setup', {
      ...
      body: JSON.stringify({
        ...
        date_of_birth: sessionStorage.getItem('owner_date_of_birth'),
    ...
    if (clearSession) {
      ['owner_user_id', 'owner_full_name', 'owner_email', 'owner_password', 'owner_phone', 'owner_date_of_birth',
        'company_name', 'company_description', 'company_location', 'company_address', 'company_postal',
        'company_industry', 'company_size', 'departments', 'owner_photo_url'].forEach((k) =>
  ...
  const handleFreePlan = async () => {
    ...
    try {
      ...
      if (sessionStorage.getItem('owner_company_id')) {
        ['owner_user_id', 'owner_full_name', 'owner_email', 'owner_password', 'owner_phone', 'owner_date_of_birth', 'owner_company_id',
          'company_name', 'company_description', 'company_location', 'company_address', 'company_postal',
          'company_industry', 'company_size', 'departments', 'owner_photo_url'].forEach(k => sessionStorage.removeItem(k));
      } else {
        if (profilePhotoFile && !sessionStorage.getItem('owner_photo_url')) {
          const url = await uploadProfilePhoto(profilePhotoFile);
          if (url) sessionStorage.setItem('owner_photo_url', url);
        }
