...
import {
  ...
  Copy, UserRound, Pencil, Trash2, CalendarDays, ChevronLeft, ChevronRight,
} from 'lucide-react'
        ...
        {cells.map((date, i) => {
          ...
          return (
            <button key={date} type="button" onClick={() => { onChange(date); setOpen(false) }}
              style={{ height: 36, width: '100%', border: isToday && !isSel ? `2px solid ${TASK_GREEN}` : 'none', borderRadius: 8, background: isSel ? TASK_GREEN : 'transparent', color: isSel ? '#FFFFFF' : isToday ? TASK_GREEN : TASK_TEXT, fontWeight: isSel || isToday ? 700 : 400, fontSize: 13, cursor: 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 2, padding: 0 }}
              onMouseEnter={e => { if (!isSel) e.currentTarget.style.background = '#F8FAFC' }}
              onMouseLeave={e => { if (!isSel) e.currentTarget.style.background = 'transparent' }}
            >
              <span style={{ lineHeight: 1 }}>{parseInt(date.split('-')[2])}</span>
              {hasTask && <span style={{ width: 4, height: 4, borderRadius: '50%', background: isPast ? '#94A3B8' : isSel ? 'rgba(255,255,255,0.8)' : TASK_GREEN, flexShrink: 0 }} />}
            </button>
          )
        })}
      </div>
    ...
    <div style={{ position: 'relative' }}>
      <button ref={triggerRef} type="button" disabled={disabled}
        ...
        style={{
          ...
          padding: '10px 12px', border: `1.5px solid ${open ? TASK_GREEN : '#E5E7EB'}`, borderRadius: 8,
            ...
            return (
              <button key={opt.value} type="button"
                ...
                style={{
                  ...
                  border: 'none', background: isSel ? '#F0FDF4' : 'transparent',
                  color: isSel ? TASK_GREEN : '#374151', fontWeight: isSel ? 700 : 400,
...
function TaskCard({
  ...
  const MENU_ITEMS = [
    { label: 'Edit',      icon: <Pencil size={13} style={{ color: TASK_GREEN }} />,  action: onEdit,      color: '#374151' },
    { label: 'Duplicate', icon: <Copy size={13} style={{ color: TASK_GREEN }} />,    action: onDuplicate, color: '#374151' },
  ]
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, minWidth: 0 }}>
            <div style={{ display: 'flex', flexShrink: 0 }}>
              {assignees.slice(0, 3).map((assignee, i) => (
                <div key={assignee.id} className="task-card-icon" style={{ width: 22, height: 22, borderRadius: '50%', background: '#F0FDF4', border: '1.5px solid #16A34A', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, marginLeft: i === 0 ? 0 : -6, boxShadow: i === 0 ? undefined : '0 0 0 1.5px #FFFFFF' }}>
...
export default function EmployeeTasksPage() {
  ...
  // Review sign-off (Approve / Reject) on tasks this employee assigned to Casual Workers.
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      if (data.company) {
        if (!cancelled) { setCompanyId(data.company.id); setInitialReady(true) }
      } else {
  ...
  useEffect(() => {
    ...
    ]).then(([deptData, memberData, mgrData]) => {
      const self = memberData.success ? memberData.members.find((m: Member) => m.id === internalUserId) : null
      const ownDeptId = self?.department_id ?? ''
      if (deptData.success) setDepartments(deptData.departments.filter((d: Department) => !ownDeptId || d.id === ownDeptId))
      if (memberData.success) setMembers(memberData.members.filter((m: Member) => !ownDeptId || m.department_id === ownDeptId))
      if (mgrData.success) {
  ...
  // ── Review sign-off on tasks this employee assigned (Employee → Casual Worker) ──
  ...
  const filteredTasks = (col: Task['status']): Task[] => {
    ...
    return (kanban[col] ?? [])
      ...
      .filter(t => !selectedCWId || (t.assigned_user_ids ?? (t.assigned_user_id ? [t.assigned_user_id] : [])).includes(selectedCWId))
      .filter(t => {
  ...
  // EP assigns tasks to Casual Workers
  ...
  const primaryBtn = (loading: boolean): React.CSSProperties => ({
    flex: 1, padding: '10px', background: TASK_GREEN, border: 'none', borderRadius: '8px',
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', background: '#F7F8FA', fontFamily: "'Inter', system-ui, -apple-system, sans-serif" }}>
      ...
      <style>{`
        ...
        @keyframes iconBounce { 0%, 100% { transform: translateY(0); } 40% { transform: translateY(-3px); } 70% { transform: translateY(-1px); } }
        .task-card { transition: box-shadow 0.16s ease, transform 0.16s ease, border-color 0.16s ease; }
        .task-card:hover { box-shadow: 0 6px 22px rgba(0,0,0,0.10), 0 0 0 1.5px rgba(22,163,74,0.15) !important; transform: translateY(-2px); }
        .task-card:hover .task-card-icon { animation: iconBounce 0.45s ease forwards; }
        .kanban-col { transition: box-shadow 0.18s ease; }
        .kanban-col:hover { box-shadow: 0 4px 18px rgba(0,0,0,0.06); }
        .dept-pill { transition: all 0.13s ease; }
        .dept-pill:hover { transform: translateY(-1px); box-shadow: 0 2px 8px rgba(0,0,0,0.08); }
        .member-card { transition: box-shadow 0.15s ease, transform 0.15s ease, border-color 0.15s ease; }
        .member-card:hover { box-shadow: 0 4px 14px rgba(0,0,0,0.07); transform: translateX(2px); border-color: #86EFAC !important; }
        .assign-task-btn { transition: background 0.13s ease, transform 0.13s ease, box-shadow 0.13s ease; }
        .assign-task-btn:hover { background: #DCFCE7 !important; border-color: #86EFAC !important; transform: scale(1.12); box-shadow: 0 2px 8px rgba(22,163,74,0.22); }
        .new-task-btn { transition: background 0.15s ease, transform 0.12s ease, box-shadow 0.15s ease; }
        .new-task-btn:hover { background: #15803D !important; transform: translateY(-1px); box-shadow: 0 4px 14px rgba(22,163,74,0.30); }
        .today-btn { transition: background 0.15s ease, color 0.15s ease, transform 0.12s ease; }
        .today-btn:hover { transform: translateY(-1px); }
      ...
      <main style={{ marginLeft: '64px', flex: 1, height: '100vh', overflowY: 'auto', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ padding: '20px 28px 16px', flexShrink: 0, display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 24 }}>
          <div>
            <h1 className="mb-0 font-heading text-3xl font-bold tracking-tight text-gray-950">Tasks</h1>
          </div>
          ...
          <div style={{ background: '#FFFFFF', borderRadius: 16, border: '1px solid #E5E7EB', boxShadow: '0 1px 6px rgba(0,0,0,0.06)', flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', minHeight: 0 }}>
            ...
            <div style={{ padding: '14px 20px', borderBottom: '1px solid #F3F4F6', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0, gap: 12, flexWrap: 'wrap' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
                  <UserRound size={15} style={{ color: TASK_GREEN }} />
                  <span style={{ fontSize: 14, fontWeight: 700, color: '#0F172A', letterSpacing: '-0.2px' }}>Casual Worker:</span>
                </div>
                <button
                  onClick={() => setSelectedCWId('')}
                  className="dept-pill"
                  style={{ padding: '5px 13px', borderRadius: '99px', border: selectedCWId === '' ? `2px solid ${TASK_GREEN}` : '1.5px solid #E5E7EB', background: selectedCWId === '' ? TASK_GREEN : 'transparent', color: selectedCWId === '' ? '#FFFFFF' : '#374151', fontWeight: 600, fontSize: '0.8rem', cursor: 'pointer', flexShrink: 0 }}
                >
                  All
                </button>
                {assignableMembers.map(m => (
                  <button
                    key={m.id}
                    onClick={() => setSelectedCWId(selectedCWId === m.id ? '' : m.id)}
                    className="dept-pill"
                    style={{ padding: '5px 13px', borderRadius: '99px', border: selectedCWId === m.id ? `2px solid ${TASK_GREEN}` : '1.5px solid #E5E7EB', background: selectedCWId === m.id ? TASK_GREEN : 'transparent', color: selectedCWId === m.id ? '#FFFFFF' : '#374151', fontWeight: 600, fontSize: '0.8rem', cursor: 'pointer', flexShrink: 0 }}
                  >
                    {m.full_name}
                  </button>
                ))}
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
                <button
                  ...
                  className="today-btn"
                  style={{ height: 38, padding: '0 14px', border: `1px solid ${TASK_BORDER}`, borderRadius: 8, background: taskDate === formatDateKey(new Date()) ? TASK_GREEN : '#FFFFFF', color: taskDate === formatDateKey(new Date()) ? '#FFFFFF' : TASK_TEXT, fontSize: 13, fontWeight: 700, cursor: 'pointer' }}
                >Today</button>
                <button
                  type="button"
                ...
                {selectedCWId && (
                  <button
                    onClick={() => {
                      const cw = members.find(m => m.id === selectedCWId)
                      setNewTitle(''); setNewDescription(''); setNewPriority(''); setNewShiftId(''); setNewDeadlineTime(''); setNewError('')
                      setNewAssigneeIds([selectedCWId])
                      setNewDeptId(cw?.department_id ?? departments[0]?.id ?? '')
                      setNewTaskModal(true)
                    }}
                    className="new-task-btn"
                    style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '7px 14px', background: TASK_GREEN, border: 'none', borderRadius: 8, fontWeight: 600, fontSize: '0.8125rem', color: '#fff', cursor: 'pointer', height: 38, flexShrink: 0 }}
                  >
            ...
            {!initialReady || kanbanLoading ? (
              ...
              <div style={{ display: 'flex', flex: 1, minHeight: 0, overflow: 'hidden' }}>
                ...
                {/* ── LEFT SIDEBAR: CW list (only when no CW filter active) ── */}
                {!selectedCWId && departments.length > 0 && (() => {
                  const cwMembers = members.filter(m => m.role === 'Casual Worker')
                  ...
                  const freeMembers = cwMembers.filter(m => !busyIds.has(m.id))
                  const busyMembers = cwMembers.filter(m => busyIds.has(m.id))

                  if (cwMembers.length === 0) return null
                  ...
                  const renderMember = (m: Member) => (
                    <div key={m.id} className="member-card" style={{ display: 'flex', alignItems: 'center', gap: 8, border: '1px solid #F1F5F9', borderRadius: 12, padding: '9px 10px', background: '#FFFFFF', justifyContent: 'space-between' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 7, minWidth: 0 }}>
                        <span style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: 26, height: 26, borderRadius: 999, background: '#F0FDF4', color: TASK_GREEN, flexShrink: 0 }}>
                          <UserRound size={13} />
                        </span>
                      ...
                      <button
                        ...
                        style={{ flexShrink: 0, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: 26, height: 26, background: '#F0FDF4', border: '1px solid #86EFAC', borderRadius: 7, color: TASK_GREEN, cursor: 'pointer' }}
                        title="Assign Task"
                      >
                      ...
                      <Fragment key={col}>
                        ...
                        <div className="kanban-col" style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', background: '#F7F8FA', borderRadius: '12px', overflow: 'hidden', minHeight: 0, border: '1px solid #F0F1F3', animationDelay: `${0.06 + colIdx * 0.05}s` }}>
                          ...
                          <div
                            ...
                            style={{ flex: 1, overflowY: 'auto', padding: '10px 10px 12px', background: dragOverCol === col ? '#F0FDF4' : undefined, outline: dragOverCol === col ? `2px dashed ${TASK_GREEN}` : 'none', outlineOffset: -4, transition: 'background 0.15s' }}
                          >
                                              ...
                                              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flexShrink: 0 }}>
                                                <button
                                                  ...
                                                  style={{
                                                    ...
                                                    border: sub.is_completed ? 'none' : `1.5px solid ${clickable ? TASK_GREEN : '#CBD5E1'}`,
                                                    background: sub.is_completed ? '#16A34A' : '#FFFFFF',
                                                    color: sub.is_completed ? '#FFFFFF' : clickable ? TASK_GREEN : '#CBD5E1',
      ...
      {selectedTask && taskViewMode && (() => {
        ...
        const viewDeadline = selectedTask.due_at
          ? new Date(selectedTask.due_at).toLocaleString('en-US', { weekday: 'short', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })
          : null
        const viewFieldValue: React.CSSProperties = { padding: '10px 12px', border: '1.5px solid #E5E7EB', borderRadius: 8, background: '#F9FAFB', fontSize: '0.9375rem', color: '#111827', minHeight: 40, display: 'flex', alignItems: 'center' }
        ...
        return (
          <div onClick={closePanel} style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.45)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50 }}>
            <div ref={panelRef} onClick={e => e.stopPropagation()} style={{ width: 540, background: '#FFFFFF', borderRadius: 20, overflow: 'hidden', boxShadow: '0 24px 64px rgba(0,0,0,0.16)' }}>
              <div style={{ padding: '20px 24px 18px', borderBottom: '1px solid #F3F4F6', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                ...
                <button onClick={closePanel} style={{ background: '#F9FAFB', border: '1px solid #E5E7EB', cursor: 'pointer', color: '#6B7280', display: 'flex', padding: '6px', borderRadius: 8 }}><X size={16} /></button>
              </div>
              <div style={{ padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 16, maxHeight: 'calc(90vh - 130px)', overflowY: 'auto' }}>
                <div><label style={modalLabelStyle}>Title</label><div style={viewFieldValue}>{selectedTask.title}</div></div>
                <div><label style={modalLabelStyle}>Description</label>{selectedTask.description ? <div style={{ ...viewFieldValue, alignItems: 'flex-start', whiteSpace: 'pre-wrap', lineHeight: 1.55 }}>{selectedTask.description}</div> : <div style={viewEmpty}>No description</div>}</div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                  <div><label style={modalLabelStyle}>Department</label><div style={viewDept ? viewFieldValue : viewEmpty}>{viewDept?.name ?? 'Unknown'}</div></div>
                ...
                <div>
                  <label style={modalLabelStyle}><span style={{ display: 'flex', alignItems: 'center', gap: 5 }}><Clock size={12} color={TASK_GREEN} />Deadline</span></label>
                  {viewDeadline ? <div style={viewFieldValue}>{viewDeadline}</div> : <div style={viewEmpty}>No deadline set</div>}
                </div>
      ...
      {selectedTask && !taskViewMode && (
        <div onClick={closePanel} style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.45)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50 }}>
          <div ref={panelRef} onClick={e => e.stopPropagation()} data-testid="task-detail-panel"
            style={{ width: 540, background: '#FFFFFF', borderRadius: 20, overflow: 'hidden', boxShadow: '0 24px 64px rgba(0,0,0,0.16)' }}
          >
            <div style={{ padding: '20px 24px 18px', borderBottom: '1px solid #F3F4F6', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <div style={{ width: 32, height: 32, borderRadius: 9, background: 'linear-gradient(135deg, #16A34A, #15803D)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            ...
            <div style={{ padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 16, maxHeight: 'calc(90vh - 140px)', overflowY: 'auto' }}>
              <div>
                <label style={modalLabelStyle}>Title <span style={{ color: TASK_GREEN }}>*</span></label>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                ...
                <div>
                  <label style={modalLabelStyle}>Assign To (CW)</label>
              ...
              <div>
                <label style={modalLabelStyle}>
                  ...
                  {editAssigneeIds.length > 0 && editAssigneeShiftOptions.length === 0 && (
                    <span style={{ fontWeight: 400, color: TASK_GREEN, marginLeft: 8, fontSize: '0.78rem' }}>no upcoming shifts</span>
                  )}
                </label>
                <DropdownField value={editShiftId} options={editShiftDropdownOptions} onChange={v => { setEditShiftId(v); const s = editAssigneeShiftOptions.find(x => x.id === v); if (s) setEditDeadlineTime(s.end_time.slice(0, 5)) }} placeholder={editAssigneeIds.length > 0 ? 'Select shift' : 'Select assignee first'} disabled={editAssigneeIds.length === 0} />
              </div>
              <div>
                <label style={modalLabelStyle}>
                  <span style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                    <Clock size={12} color={TASK_GREEN} />
                    Deadline
              ...
              <div>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  ...
                  <button onClick={handleCreateSubTask} disabled={subTaskLoading || !subTaskTitle.trim()} style={{ padding: '0 12px', border: 'none', borderRadius: 8, background: TASK_GREEN, color: '#FFFFFF', fontWeight: 700, fontSize: '0.82rem', cursor: subTaskLoading || !subTaskTitle.trim() ? 'default' : 'pointer', opacity: subTaskLoading || !subTaskTitle.trim() ? 0.6 : 1 }}>
                    Add
                  </button>
                </div>
              </div>
            ...
            <div style={{ padding: '0 24px 20px', display: 'flex', gap: 10 }}>
              {!deleteConfirm ? (
                <>
                  <button onClick={() => setDeleteConfirm(true)} style={{ padding: '8px 14px', background: 'none', border: '1.5px solid #FECACA', borderRadius: 8, fontWeight: 600, fontSize: '0.875rem', color: '#DC2626', cursor: 'pointer' }}>Delete</button>
                  ...
                  <div style={{ flex: 1, display: 'flex', gap: 10 }}>
                    ...
                    <button style={{ ...primaryBtn(editLoading), background: editLoading ? '#16A34A' : 'linear-gradient(135deg, #16A34A, #15803D)', border: 'none' }} onClick={handleSaveTask} disabled={editLoading}>
      ...
      {deleteTaskModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.42)', display: 'grid', placeItems: 'center', padding: 18, zIndex: 100 }}>
          <div onClick={e => e.stopPropagation()} style={{ width: 'min(480px, 100%)', maxHeight: '88vh', overflowY: 'auto', background: '#FFFFFF', borderRadius: 16, border: '1px solid #E2E8F0', boxShadow: '0 24px 70px rgba(15,23,42,0.32)', padding: 16 }}>
            ...
            <p style={{ color: '#64748B', marginTop: 0, fontSize: 13 }}>Are you sure you want to delete <strong style={{ color: TASK_TEXT }}>{deleteTaskModal.title}</strong>?</p>
      ...
      {newTaskModal && (
        <div onClick={() => setNewTaskModal(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.45)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
          <div onClick={e => e.stopPropagation()} style={{ width: 540, background: '#FFFFFF', borderRadius: 20, overflow: 'hidden', boxShadow: '0 24px 64px rgba(0,0,0,0.16)' }}>
            <div style={{ padding: '20px 24px 18px', borderBottom: '1px solid #F3F4F6', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <div style={{ width: 32, height: 32, borderRadius: 9, background: 'linear-gradient(135deg, #16A34A, #15803D)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              ...
              <button onClick={() => setNewTaskModal(false)} style={{ background: '#F9FAFB', border: '1px solid #E5E7EB', cursor: 'pointer', color: '#6B7280', display: 'flex', padding: '6px', borderRadius: 8 }}><X size={16} /></button>
            </div>
            ...
            <div style={{ padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <div>
                ...
                <DropdownField value={newShiftId} options={newShiftDropdownOptions} onChange={v => { setNewShiftId(v); const s = newTaskShiftOptions.find(x => x.id === v); if (s) setNewDeadlineTime(s.end_time.slice(0, 5)) }} placeholder={newAssigneeIds.length > 0 ? 'Select shift' : 'Select assignee first'} disabled={newAssigneeIds.length === 0} />
              </div>
              <div>
                <label style={modalLabelStyle}>
                  <span style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
            ...
            <div style={{ padding: '0 24px 20px', display: 'flex', gap: 10 }}>
              ...
              <button style={{ ...primaryBtn(newLoading), background: newLoading ? '#16A34A' : 'linear-gradient(135deg, #16A34A, #15803D)', border: 'none' }} onClick={handleCreateTask} disabled={newLoading}>
