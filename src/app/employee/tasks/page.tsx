...
export default function EmployeeTasksPage() {
  ...
  const [headerTheme] = useState<{ bg: string; text: string; border: string }>(() => {
    ...
    return role === 'Partner'
      ...
      : role === 'Employee'
        ? { bg: '#166534', text: '#FFFFFF', border: '1px solid #14532D' }
  ...
  const canManageDepartments = false
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      const self = memberData.success ? memberData.members.find((member: Member) => member.id === internalUserId) : null
      const ownDeptId = self?.department_id ?? ''
      if (deptData.success) setDepartments(deptData.departments.filter((dept: Department) => !ownDeptId || dept.id === ownDeptId))
      if (memberData.success) setMembers(memberData.members.filter((member: Member) => !ownDeptId || member.department_id === ownDeptId))
      if (mgrData.success) {
      ...
      if (statsData.success) setDeptTaskStats((statsData.dept_stats ?? []).filter((stat: DeptTaskStats) => !ownDeptId || stat.department_id === ownDeptId))
    }).catch(() => {})
  ...
  const filteredTasks = (col: Task['status']): Task[] => {
    ...
    return (kanban[col] ?? []).filter(t => {
      ...
      if (t.assigned_user_id !== internalUserId && t.assigned_by !== internalUserId) return false
        ...
        <div onClick={() => setNewTaskModal(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', backdropFilter: 'blur(2px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
          <div onClick={e => e.stopPropagation()} style={{ width: '540px', background: '#FFFFFF', borderRadius: '16px', padding: '32px', boxShadow: '0 8px 40px rgba(0,0,0,0.12)' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                <div>
                  <label style={modalLabelStyle}>Assign To (Casual Worker)</label>
                  <div style={{ position: 'relative' }}>
