...
import { Clock, Plus, Check, AlertTriangle, RefreshCw } from 'lucide-react'
...
export default function EmployeeSettingsPage() {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      const leaveRes = await fetch(`/api/user/leave-requests?user_id=${internalId}`)
            ...
            <div style={{ padding: '18px 20px', display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 5, flex: '0 0 180px' }}>
                  ...
                  <select value={leaveType} onChange={e => setLeaveType(e.target.value)} style={{ height: 36, border: `1px solid ${BORDER}`, borderRadius: 8, padding: '0 10px', fontSize: 13, color: TEXT, background: '#FFFFFF', outline: 'none' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 5, flex: 1, minWidth: 200 }}>
                  ...
                  <input type="text" value={leaveReason} onChange={e => setLeaveReason(e.target.value)} placeholder="Brief description..." style={{ height: 36, border: `1px solid ${BORDER}`, borderRadius: 8, padding: '0 10px', fontSize: 13, color: TEXT, background: '#FFFFFF', outline: 'none' }} />
                </div>
              </div>
              {leaveMsg && (
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
                <button type="button" onClick={submitLeave} disabled={submittingLeave} style={{ height: 36, padding: '0 20px', borderRadius: 8, border: 'none', background: '#F97316', color: '#FFFFFF', fontWeight: 700, fontSize: 13, cursor: submittingLeave ? 'not-allowed' : 'pointer', opacity: submittingLeave ? 0.7 : 1, display: 'inline-flex', alignItems: 'center', gap: 6 }}>
