...
export default function EmployeeCommunicationPage() {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel('employee-comm-announcements')
  ...
  useEffect(() => {
    ...
    setPendingPrefill('')
    window.history.replaceState({}, '', '/employee/communication')
    ...
    fetch(`/api/team/members?company_id=${companyId}${userDeptId ? `&department_id=${userDeptId}` : ''}`)
      .then(r => r.json())
      .then(data => {
        if (!data.success) return
        const members = (data.members as CompanyMember[]).filter(
          m => (m.role === 'Manager' || m.role === 'Employee') && m.id !== internalUserId
        )
        setCompanyMembers(members)
        const partner = members.find(m => m.id === pid)
        if (partner) {
  ...
  useEffect(() => {
    if (!internalUserId) return
    const channel = supabase
      .channel('employee-comm-messages')
  ...
  const openCompose = useCallback(() => {
    ...
    fetch(`/api/team/members?company_id=${companyId}${userDeptId ? `&department_id=${userDeptId}` : ''}`)
      ...
      .then(data => {
        if (!data.success) return
        const eligible = (data.members as CompanyMember[]).filter(
          m => (m.role === 'Manager' || m.role === 'Employee') && m.id !== internalUserId
        )
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0, paddingTop: 4 }}>
            {employeeName && (
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, height: 36, background: '#fff', border: '1.5px solid #E5E7EB', borderRadius: 999, padding: '0 14px 0 6px', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
                <span style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: 26, height: 26, borderRadius: 999, background: DARK_BG, color: '#FFFFFF', flexShrink: 0 }}>
                  <UserRound size={13} />
                </span>
              ...
              <div style={{ flexShrink: 0 }}>
                {activeTab === 'chat' && (
                  <button onClick={openCompose} disabled={!communicationReady}
                    className={communicationReady ? 'emp-action-btn' : ''}
                            ...
                            >
                              ...
                              >
                                ...
                                <button
                                  ...
                                  style={{ flexShrink: 0, width: 26, height: 26, display: 'flex', alignItems: 'center', justifyContent: 'center', background: isPinned ? ACCENT_LIGHT : '#F8FAFC', border: isPinned ? `1.5px solid rgba(22,163,74,0.35)` : '1.5px solid #E2E8F0', borderRadius: 7, cursor: 'pointer', color: isPinned ? ACCENT : '#94A3B8', transition: 'all 0.15s' }}
                              ...
                              {/* Divider */}
                              <div style={{ height: 1, background: DARK_BG, margin: '8px 0 0', flexShrink: 0 }} />
                              ...
                              <div style={{ flex: 1, minHeight: 0, overflowY: 'auto', padding: '12px 14px', display: 'flex', flexDirection: 'column', gap: 7 }}>
                                {msgs.map(msg => {
                                  ...
                                  return (
                                    <div key={msg.id} className="emp-msg-bubble" style={{ display: 'flex', justifyContent: isMine ? 'flex-end' : 'flex-start', alignItems: 'flex-end', gap: 6 }}>
                              ...
                              <div style={{ padding: '8px 12px', background: '#FFFFFF', display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0 }}>
                                ...
                                {!conv.partnerDeleted && (
                                  <button onClick={() => panelPhotoRefs.current[partnerId]?.click()} title="Send photo"
                                    ...
                                    onMouseEnter={e => { e.currentTarget.style.background = ACCENT_LIGHT; e.currentTarget.style.borderColor = `rgba(22,163,74,0.35)`; e.currentTarget.style.color = ACCENT }}
                      ...
                      {filteredAnnouncements.length === 0 ? (
                        ...
                        return (
                          <button
                            ...
                            className="emp-ann-card"
                            style={{
                              display: 'flex', flexDirection: 'column', gap: 6, padding: 12,
                              background: selected ? ACCENT_LIGHT : '#FFFFFF',
                              border: selected ? `1.5px solid rgba(22,163,74,0.35)` : '1.5px solid #EDF2F7',
                              borderRadius: 12, cursor: 'pointer', textAlign: 'left', width: '100%', marginBottom: 8,
                              boxShadow: selected ? `0 6px 18px rgba(22,163,74,0.08)` : '0 1px 3px rgba(0,0,0,0.03)',
                              animationDelay: `${i * 0.04}s`,
                            }}
                          >
                            <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', gap: 12, minWidth: 0 }}>
                            ...
                            <div style={{ minWidth: 0 }}>
                              ...
                              <p style={{ margin: '4px 0 0', color: '#64748B', fontSize: 12, fontWeight: 600 }}>
                                {selectedAnn.created_by_name ?? 'Employee'} · {new Date(selectedAnn.created_at).toLocaleString([], { dateStyle: 'medium', timeStyle: 'short' })}
                              </p>
                            </div>
                          </div>
                  ...
                  {inviteLoading ? (
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(340px, 1fr))', gap: 14 }}>
                      {invites.map((invite, i) => (
                        <div key={invite.id} className="emp-ann-card" style={{ padding: 18, background: '#FFFFFF', border: '1.5px solid #EDF2F7', borderRadius: 14, animationDelay: `${i * 0.05}s` }}>
          ...
          <div style={{ background: '#fff', borderRadius: 18, width: 500, maxWidth: '92vw', boxShadow: '0 24px 70px rgba(0,0,0,0.18)', overflow: 'hidden', animation: 'fadeSlideUp 0.22s ease both' }}>
            ...
            <div style={{ padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 14 }}>
              <div>
                ...
                <input value={editTitle} onChange={e => setEditTitle(e.target.value)} placeholder="Announcement title" className="emp-input" style={inputStyle} />
              </div>
              <div>
                <label style={labelStyle}>Content</label>
                <textarea value={editContent} onChange={e => setEditContent(e.target.value)} placeholder="Write your announcement..." rows={5} className="emp-textarea" style={textareaStyle} />
              </div>
              <div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.45)', backdropFilter: 'blur(4px)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ background: '#fff', borderRadius: 18, width: 500, maxWidth: '92vw', boxShadow: '0 24px 70px rgba(0,0,0,0.18)', overflow: 'hidden', animation: 'fadeSlideUp 0.22s ease both' }}>
            ...
            <div style={{ padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 14 }}>
              <div>
                <label style={labelStyle}>Title</label>
                <input value={annTitle} onChange={e => setAnnTitle(e.target.value)} placeholder="Announcement title" className="emp-input" style={inputStyle} />
              </div>
              <div>
                <label style={labelStyle}>Content</label>
                <textarea data-testid="announcement-content" value={annContent} onChange={e => setAnnContent(e.target.value)} placeholder="Write your announcement here..." rows={5} className="emp-textarea" style={textareaStyle} />
              </div>
              <div>
                <label style={labelStyle}>Audience</label>
                <div style={{ ...inputStyle, display: 'flex', alignItems: 'center', color: '#64748B', cursor: 'default', userSelect: 'none' }}>
          ...
          <div onClick={e => e.stopPropagation()} style={{ width: 480, background: '#fff', borderRadius: 18, boxShadow: '0 24px 70px rgba(0,0,0,0.18)', display: 'flex', flexDirection: 'column', maxHeight: '88vh', overflow: 'hidden', animation: 'fadeSlideUp 0.22s ease both' }}>
            ...
            <div style={{ padding: '18px 24px', display: 'flex', flexDirection: 'column', gap: 16, overflowY: 'auto', flex: 1 }}>
              <div>
                ...
                {selectedRecipient ? (
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 13px', background: ACCENT_LIGHT, border: `1.5px solid rgba(22,163,74,0.3)`, borderRadius: 11 }}>
...
const primaryBtnStyle: React.CSSProperties = {
  flex: 1, height: 40, background: '#16A34A', border: 'none', borderRadius: 10,
