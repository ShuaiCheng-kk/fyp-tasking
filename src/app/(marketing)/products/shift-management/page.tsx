...
const pointDefaults: Record<string, string[]> = {
  '1': ['Assign to Managers or Employees', 'Set dates and shift times', 'Colour-coded by department'],
  '2': ['Save name and shift times', 'Reuse across any future shift', 'Apply in one click'],
  '3': ['Drafts become official shifts', 'Staff see it instantly', 'Unpublish anytime to revert'],
  '4': ['Daily, weekly, or custom rules', 'Generates the whole series', 'Edit or delete the entire series'],
  '5': ['Two shifts from one date', 'Built-in break window in between', 'Non-overlapping times enforced'],
  '6': ['Filter by date or department', 'Edit many shifts at once', 'Save all changes together'],
  '7': ['Guided 4-step wizard', 'Follows your workforce rules', 'Review before accepting'],
};
...
export default function ShiftManagementPage() {
  const copy = useMarketingCopy('products-shift-management');

  const featureIdxs = copy.keys('feature.', '.name');

  const stepDefaults = [
    { n: '01', titleKey: 'workflow.step1.title', descKey: 'workflow.step1.desc', defaultTitle: 'Build', defaultDesc: 'Start from scratch, reuse a template, split, or set recurring rules.' },
    { n: '02', titleKey: 'workflow.step2.title', descKey: 'workflow.step2.desc', defaultTitle: 'Assign', defaultDesc: 'Assign shifts to Managers and Employees on the timeline or calendar.' },
    { n: '03', titleKey: 'workflow.step3.title', descKey: 'workflow.step3.desc', defaultTitle: 'Publish', defaultDesc: 'One click turns the draft into an official schedule staff can see instantly.' },
    { n: '04', titleKey: 'workflow.step4.title', descKey: 'workflow.step4.desc', defaultTitle: 'Optimise', defaultDesc: 'Run an AI suggestion or bulk-edit shifts whenever plans change.' },
  ];
  ...
  return (
    <>
      ...
      <section className="sub-section" style={{ background: '#1C1C1E', padding: '96px 0 80px', overflow: 'visible', position: 'relative', zIndex: 2 }}>
        {/* Curved fade into the next section — same technique as the Home page's Hero, so the
            dark-to-light transition isn't a flat cut. Sits behind the floating screenshot below. */}
        <svg
        ...
        >
          <defs>
            <filter id="shiftHeroCurveBlur" x="-20%" y="-20%" width="140%" height="140%">
        ...
        <div className="sub-inner" style={{ ...inner, textAlign: 'center', position: 'relative' }}>
          <span style={{ display: 'inline-block', background: 'rgba(249,115,22,0.18)', color: '#FB923C', padding: '5px 14px', borderRadius: '100px', fontSize: '0.8125rem', fontWeight: 600, fontFamily: 'var(--font-body)', marginBottom: '24px' }}>
            {copy('hero.badge', 'Shift Management')}
          </span>
          <h1 className="sub-hero-h1" style={{ fontFamily: 'var(--font-heading)', fontWeight: 700, fontSize: '3rem', lineHeight: 1.15, color: '#FFFFFF', maxWidth: '680px', margin: '0 auto 36px' }}>
            {copy('hero.headline', 'Build the schedule your team can actually see.')}
          </h1>
          ...
          {/* Product demo video — floats past the hero's own bottom edge into the section below,
              matching the Homebase-style "mockup poking out of the hero" treatment. */}
          <div style={{ maxWidth: '1100px', margin: '56px auto -160px' }}>
            ...
            >
              <source src={copy('hero.video', '/Shift.mkv')} type="video/x-matroska" />
      ...
      <section className="sub-section" style={{ background: '#FFFBF5', padding: '220px 0 80px' }}>
        <div className="sub-inner" style={{ ...inner, marginBottom: '52px' }}>
          <div style={{ textAlign: 'center' }}>
            <h2 className="sub-h2" style={h2style}>{copy('features.title', 'Everything you need to run the schedule')}</h2>
            <p style={subtitleStyle}>{copy('features.subtitle', 'From building the first draft to publishing the final version.')}</p>
          </div>
        </div>
        {/* Carousel — independent cards side by side, paged with the left/right arrows. */}
      ...
      <section className="sub-section" style={{ background: '#FFFFFF', padding: '80px 0' }}>
        <div className="sub-inner" style={inner}>
          <div style={{ textAlign: 'center', marginBottom: '60px' }}>
            <h2 className="sub-h2" style={h2style}>{copy('workflow.title', 'From a blank week to a published schedule.')}</h2>
            <p style={subtitleStyle}>{copy('workflow.subtitle', 'A complete scheduling flow, built for departments with shifting availability.')}</p>
          </div>
