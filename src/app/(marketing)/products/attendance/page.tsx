...
const pointDefaults: Record<string, string[]> = {
  '1': ['10-minute grace period included', 'Blocked more than 30 min early', 'No app or hardware required'],
  '2': ['One break allowed per shift', 'Clock Out locked until break ends', 'No fixed break duration'],
  '3': ['Counterpart accepts it first', 'Employees ask their Manager', 'Managers ask Owner or Partner'],
  '4': ['Exact weekly quota required', 'Approved as-is, or modified', 'Never rejected outright'],
  '6': ['Marked Safe or Flagged', 'Flags mean understaffing risk', 'Suggests a safe replacement date'],
};
...
export default function AttendancePage() {
  ...
  const stepDefaults = [
    { n: '01', titleKey: 'workflow.step1.title', descKey: 'workflow.step1.desc', defaultTitle: 'Clock In', defaultDesc: "Clock in within 30 minutes of the shift's start, tracked as Present or Late automatically." },
    { n: '02', titleKey: 'workflow.step2.title', descKey: 'workflow.step2.desc', defaultTitle: 'Break', defaultDesc: 'Start and end one break mid-shift whenever needed, no fixed window required.' },
    { n: '03', titleKey: 'workflow.step3.title', descKey: 'workflow.step3.desc', defaultTitle: 'Clock Out', defaultDesc: 'Clock out once the shift ends, or once your supervisor releases you on an open-ended job.' },
    { n: '04', titleKey: 'workflow.step4.title', descKey: 'workflow.step4.desc', defaultTitle: 'Request & Review', defaultDesc: 'Submit a swap or day-off request; the right approver decides, Manager or Owner/Partner.' },
  ];
  ...
  return (
    <>
      ...
      <section className="sub-section" style={{ background: '#1C1C1E', padding: '96px 0 80px', overflow: 'visible', position: 'relative', zIndex: 2 }}>
        ...
        >
          <defs>
            <filter id="attendanceHeroCurveBlur" x="-20%" y="-20%" width="140%" height="140%">
          ...
          <path
            ...
            fill="#FFFBF5"
            filter="url(#attendanceHeroCurveBlur)"
          />
        </svg>
        ...
        <div className="sub-inner" style={{ ...inner, textAlign: 'center', position: 'relative' }}>
          ...
          <h1 className="sub-hero-h1" style={{ fontFamily: 'var(--font-heading)', fontWeight: 700, fontSize: '3rem', lineHeight: 1.15, color: '#FFFFFF', maxWidth: '680px', margin: '0 auto 36px' }}>
            {copy('hero.headline', 'Clock in and out, without the paperwork.')}
          </h1>
          ...
          <div style={{ maxWidth: '1100px', margin: '56px auto -160px' }}>
            ...
            >
              <source src={copy('hero.video', '/Attendance.mkv')} type="video/x-matroska" />
      ...
      <section className="sub-section" style={{ background: '#FFFBF5', padding: '220px 0 80px' }}>
        <div className="sub-inner" style={{ ...inner, marginBottom: '52px' }}>
          <div style={{ textAlign: 'center' }}>
            <h2 className="sub-h2" style={h2style}>{copy('features.title', 'Everything you need to track attendance')}</h2>
            <p style={subtitleStyle}>{copy('features.subtitle', 'From clocking in to correcting a record after the fact.')}</p>
          </div>
        </div>
      ...
      <section className="sub-section" style={{ background: '#FFFFFF', padding: '80px 0' }}>
        <div className="sub-inner" style={inner}>
          <div style={{ textAlign: 'center', marginBottom: '60px' }}>
            <h2 className="sub-h2" style={h2style}>{copy('workflow.title', 'From clocking in to a corrected record.')}</h2>
            <p style={subtitleStyle}>{copy('workflow.subtitle', 'A complete attendance flow, with the right approver for every request.')}</p>
          </div>
