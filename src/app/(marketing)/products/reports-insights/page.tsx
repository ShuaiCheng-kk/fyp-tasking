...
const pointDefaults: Record<string, string[]> = {
  '1': ['Attendance, task, and workload', 'Any date range you choose', 'Broken down by department'],
  '2': ['Every figure vs. the prior period', 'See change, not just a snapshot', 'No manual comparison needed'],
  '3': ['Track your casual worker pool', 'Alongside your internal team', 'One view, not two reports'],
  '4': ['Flags concentration and swings', 'Catches hiring gaps early', "Beyond what charts can show"],
};
...
export default function ReportsInsightsPage() {
  const copy = useMarketingCopy('products-reports-insights');

  const featureIdxs = copy.keys('feature.', '.name');

  const stepDefaults = [
    { n: '01', titleKey: 'workflow.step1.title', descKey: 'workflow.step1.desc', defaultTitle: 'Select', defaultDesc: 'Choose a date range and, if you need it, a department filter.' },
    { n: '02', titleKey: 'workflow.step2.title', descKey: 'workflow.step2.desc', defaultTitle: 'Review', defaultDesc: 'See the overview, department breakdown, workload, and casual worker sections instantly.' },
    { n: '03', titleKey: 'workflow.step3.title', descKey: 'workflow.step3.desc', defaultTitle: 'Investigate', defaultDesc: 'Let AI surface anomalies your charts structurally can\'t show on their own.' },
    { n: '04', titleKey: 'workflow.step4.title', descKey: 'workflow.step4.desc', defaultTitle: 'Export', defaultDesc: 'Download the complete report as a PDF, every chart turned into a data table.' },
  ];
  ...
  return (
    <>
      ...
      <section className="sub-section" style={{ background: '#1C1C1E', padding: '96px 0 80px', overflow: 'visible', position: 'relative', zIndex: 2 }}>
        ...
        >
          <defs>
            <filter id="reportsHeroCurveBlur" x="-20%" y="-20%" width="140%" height="140%">
          ...
          <path
            ...
            fill="#FFFBF5"
            filter="url(#reportsHeroCurveBlur)"
          />
        </svg>
        ...
        <div className="sub-inner" style={{ ...inner, textAlign: 'center', position: 'relative' }}>
          <span style={{ display: 'inline-block', background: 'rgba(249,115,22,0.18)', color: '#FB923C', padding: '5px 14px', borderRadius: '100px', fontSize: '0.8125rem', fontWeight: 600, fontFamily: 'var(--font-body)', marginBottom: '24px' }}>
            {copy('hero.badge', 'Reports & Insights')}
          </span>
          <h1 className="sub-hero-h1" style={{ fontFamily: 'var(--font-heading)', fontWeight: 700, fontSize: '3rem', lineHeight: 1.15, color: '#FFFFFF', maxWidth: '680px', margin: '0 auto 36px' }}>
            {copy('hero.headline', 'Turn workforce data into decisions.')}
          </h1>
          ...
          <div style={{ maxWidth: '1100px', margin: '56px auto -160px' }}>
            ...
            >
              <source src={copy('hero.video', '/Report.mkv')} type="video/x-matroska" />
      ...
      <section className="sub-section" style={{ background: '#FFFBF5', padding: '220px 0 80px' }}>
        <div className="sub-inner" style={{ ...inner, marginBottom: '52px' }}>
          <div style={{ textAlign: 'center' }}>
            <h2 className="sub-h2" style={h2style}>{copy('features.title', 'Everything you need to understand your workforce')}</h2>
            <p style={subtitleStyle}>{copy('features.subtitle', 'Every figure compared against the period before it, plus AI to catch what the charts can\'t.')}</p>
          </div>
        </div>
      ...
      <section className="sub-section" style={{ background: '#FFFFFF', padding: '80px 0' }}>
        <div className="sub-inner" style={inner}>
          <div style={{ textAlign: 'center', marginBottom: '60px' }}>
            <h2 className="sub-h2" style={h2style}>{copy('workflow.title', 'From raw activity to a report you can share.')}</h2>
            <p style={subtitleStyle}>{copy('workflow.subtitle', 'A complete reporting flow, from filtering the data to exporting it.')}</p>
          </div>
