...
const pointDefaults: Record<string, string[]> = {
  '1': ['Company-wide or one department', 'Every post is time-stamped', 'Read by the right audience only'],
  '3': ['One-on-one with a teammate', "Scoped to your role's contacts", 'Full conversation history kept'],
  '4': ['A different list per role', 'Owner down to Casual Worker', 'No manual contact management'],
  '5': ['Managers post to their dept', 'Owner and Partner post to all', 'Reach matches your role'],
};
...
export default function CommunicationPage() {
  const copy = useMarketingCopy('products-communication');

  const featureIdxs = copy.keys('feature.', '.name');

  const stepDefaults = [
    { n: '01', titleKey: 'workflow.step1.title', descKey: 'workflow.step1.desc', defaultTitle: 'Post', defaultDesc: 'Share an announcement company-wide or to a single department.' },
    { n: '02', titleKey: 'workflow.step2.title', descKey: 'workflow.step2.desc', defaultTitle: 'Reach', defaultDesc: 'The right audience sees it instantly, scoped to their role and department.' },
    { n: '03', titleKey: 'workflow.step3.title', descKey: 'workflow.step3.desc', defaultTitle: 'Message', defaultDesc: 'Start a direct conversation with anyone in your role\'s contact scope.' },
    { n: '04', titleKey: 'workflow.step4.title', descKey: 'workflow.step4.desc', defaultTitle: 'Stay Updated', defaultDesc: 'Edit or delete your own posts anytime, with every conversation kept in one place.' },
  ];
  ...
  return (
    <>
      ...
      <section className="sub-section" style={{ background: '#1C1C1E', padding: '96px 0 80px', overflow: 'visible', position: 'relative', zIndex: 2 }}>
        ...
        >
          <defs>
            <filter id="commsHeroCurveBlur" x="-20%" y="-20%" width="140%" height="140%">
        ...
        <div className="sub-inner" style={{ ...inner, textAlign: 'center', position: 'relative' }}>
          ...
          <h1 className="sub-hero-h1" style={{ fontFamily: 'var(--font-heading)', fontWeight: 700, fontSize: '3rem', lineHeight: 1.15, color: '#FFFFFF', maxWidth: '680px', margin: '0 auto 36px' }}>
            {copy('hero.headline', 'Keep everyone in the loop. In one place.')}
          </h1>
          ...
          <div style={{ maxWidth: '1100px', margin: '56px auto -160px' }}>
            ...
            >
              <source src={copy('hero.video', '/Communication.mkv')} type="video/x-matroska" />
      ...
      <section className="sub-section" style={{ background: '#FFFBF5', padding: '220px 0 80px' }}>
        <div className="sub-inner" style={{ ...inner, marginBottom: '52px' }}>
          <div style={{ textAlign: 'center' }}>
            <h2 className="sub-h2" style={h2style}>{copy('features.title', 'Everything you need to keep your team in sync')}</h2>
            <p style={subtitleStyle}>{copy('features.subtitle', 'One module for announcements and direct messages, not two separate tools.')}</p>
          </div>
        </div>
      ...
      <section className="sub-section" style={{ background: '#FFFFFF', padding: '80px 0' }}>
        <div className="sub-inner" style={inner}>
          <div style={{ textAlign: 'center', marginBottom: '60px' }}>
            <h2 className="sub-h2" style={h2style}>{copy('workflow.title', 'From a quick update to a real conversation.')}</h2>
            <p style={subtitleStyle}>{copy('workflow.subtitle', 'Announcements and messaging, scoped correctly for every role.')}</p>
          </div>
