...
function getReadIdsKey(companyId: string, userId: string) { return `ann_read_ids_${companyId}_${userId}` }
function loadReadIds(companyId: string, userId: string): Set<string> {
  try { const raw = localStorage.getItem(getReadIdsKey(companyId, userId)); if (raw) return new Set(JSON.parse(raw)) } catch {}
  return new Set()
}
...
export default function PartnerAnnouncementsPage() {
  ...
  useEffect(() => {
    ...
    if (uid) {
      fetch(`/api/user/me?user_id=${uid}`).then(r => r.json()).then(d => {
        if (d.success) {
          setInternalUserId(d.user.id); setUserRole(d.user.role ?? ''); setUserDeptId(d.user.department_id ?? null)
  ...
  useEffect(() => {
    if (!companyId) return
    fetch(`/api/company/departments?company_id=${companyId}`).then(r => r.json()).then(d => { if (d.success) setDepartments(d.departments ?? []) }).catch(() => {})
  }, [companyId])

  const fetchUnreadMsgCount = useCallback(() => {
    if (!internalUserId || !companyId) return
    fetch(`/api/inbox/unread-count?user_id=${internalUserId}&company_id=${companyId}`).then(r => r.json()).then(d => { if (d.success) setUnreadMessages(d.unread_messages ?? 0) })
  ...
  const fetchAnnouncements = useCallback(() => {
    ...
    fetch(`/api/inbox/announcements?${params}`).then(r => r.json()).then(d => { if (d.success) setAnnouncements(d.announcements ?? []) })
  }, [companyId, userRole, userDeptId])

  useEffect(() => {
    if (!internalUserId || !companyId || !userRole) return
    fetchAnnouncements(); fetchUnreadMsgCount()
  }, [internalUserId, companyId, userRole, fetchAnnouncements, fetchUnreadMsgCount])

  useEffect(() => {
    if (!companyId) return
    const channel = supabase.channel('partner-announcements-page')
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'announcements', filter: `company_id=eq.${companyId}` }, () => { fetchAnnouncements() })
      .subscribe()
  ...
  function handleSelectAnn(ann: Announcement) {
    ...
    const next = new Set(readIds); next.add(ann.id); setReadIds(next); saveReadIds(companyId, internalUserId, next)
  }
  ...
  async function handlePost() {
    ...
    setPosting(true)
    try {
      const res = await fetch('/api/inbox/announcements', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ from_user_id: internalUserId, company_id: companyId, department_id: annDeptId === 'company-wide' ? null : annDeptId, title: annTitle, content: annContent, user_role: userRole }) })
      const data = await res.json()
      if (data.success) { setAnnouncements(prev => [data.announcement, ...prev]); setShowModal(false); setAnnTitle(''); setAnnContent(''); setAnnDeptId('company-wide') }
  ...
  async function handleDelete(announcementId: string) {
    ...
    setDeleting(true)
    try {
      const res = await fetch('/api/inbox/announcements', { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ announcement_id: announcementId, requesting_user_id: internalUserId }) })
      const data = await res.json()
  ...
  function handleOpenEdit(ann: Announcement) {
    setEditTitle(ann.title); setEditContent(ann.content)
    if (ann.department_id) { setEditAudience('specific-dept'); setEditDeptId(ann.department_id) } else { setEditAudience('company-wide'); setEditDeptId(null) }
  ...
  async function handleSaveEdit() {
    ...
    try {
      ...
      const res = await fetch('/api/inbox/announcements', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ announcement_id: selectedAnn.id, requesting_user_id: internalUserId, title: editTitle, content: editContent, department_id: deptId }) })
      const data = await res.json()
      if (!data.success) throw new Error(data.error ?? 'Failed to save')
      setShowEditModal(false); fetchAnnouncements()
      setSelectedAnn(prev => prev ? { ...prev, title: editTitle, content: editContent, department_id: deptId } : prev)
    } catch (err: any) { setEditError(err.message ?? 'An error occurred') }
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', background: '#F3F4F6', fontFamily: 'system-ui, -apple-system, sans-serif' }}>
      <PartnerSidebar unreadMessages={unreadMessages} unreadAnnouncements={unreadCount} />
        ...
        <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
          <div style={{ width: '33%', minWidth: 260, maxWidth: 360, background: '#FFFFFF', borderRight: '1px solid #E5E7EB', display: 'flex', flexDirection: 'column', flexShrink: 0 }}>
            <div style={{ padding: '13px 16px', borderBottom: '1px solid #F3F4F6', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexShrink: 0 }}>
              ...
              <button onClick={() => setShowModal(true)} style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '5px 12px', background: ACCENT, color: '#fff', border: 'none', borderRadius: 6, fontSize: '0.8rem', fontWeight: 600, cursor: 'pointer' }}>
                <Plus size={13} /> New
              </button>
            </div>
            <div style={{ overflowY: 'auto', flex: 1 }}>
              {announcements.length === 0 ? (
                ...
                return (
                  <button key={ann.id} onClick={() => handleSelectAnn(ann)} style={{ display: 'flex', flexDirection: 'column', gap: 5, padding: '13px 16px', background: selectedAnn?.id === ann.id ? ACCENT_LIGHT : 'transparent', border: 'none', cursor: 'pointer', textAlign: 'left', width: '100%', borderBottom: '1px solid #F9FAFB' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                      {unread && <div style={{ width: 7, height: 7, borderRadius: '50%', background: ACCENT, flexShrink: 0 }} />}
                      <span style={{ fontWeight: unread ? 700 : 500, fontSize: '0.875rem', color: '#111827', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', flex: 1 }}>{ann.title}</span>
                      {ann.created_by_name && <span style={{ fontSize: '12px', color: '#6B7280', whiteSpace: 'nowrap', flexShrink: 0 }}>{ann.created_by_name}</span>}
                    </div>
                    ...
                    <div style={{ fontSize: '0.8rem', color: '#6B7280', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{ann.content.slice(0, 60)}{ann.content.length > 60 ? '…' : ''}</div>
                  </button>
                )
              })}
            </div>
            ...
            {selectedAnn ? (
              <div style={{ flex: 1, overflowY: 'auto', padding: 32 }}>
                <div style={{ background: '#FFFFFF', borderRadius: 12, padding: 28, boxShadow: '0 1px 4px rgba(0,0,0,0.06)', maxWidth: 720 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0, marginLeft: 16 }}>
                      ...
                      {selectedAnn.from_user_id === internalUserId && (
                        <>
                          <button onClick={() => handleOpenEdit(selectedAnn)} style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '4px 10px', background: 'none', border: '1px solid #E5E7EB', borderRadius: 6, cursor: 'pointer', color: '#374151', fontSize: '0.8rem', fontWeight: 500 }} onMouseEnter={e => { e.currentTarget.style.background = '#F9FAFB' }} onMouseLeave={e => { e.currentTarget.style.background = 'none' }}>
                            <Pencil size={13} /> Edit
                          </button>
                          <button onClick={() => setDeleteConfirmId(selectedAnn.id)} style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '4px 10px', background: 'none', border: '1px solid #FECACA', borderRadius: 6, cursor: 'pointer', color: '#DC2626', fontSize: '0.8rem', fontWeight: 500 }} onMouseEnter={e => { e.currentTarget.style.background = '#FEF2F2' }} onMouseLeave={e => { e.currentTarget.style.background = 'none' }}>
                  ...
                  <div style={{ fontSize: '0.8125rem', color: '#9CA3AF', marginBottom: 4 }}>{new Date(selectedAnn.created_at).toLocaleString([], { dateStyle: 'medium', timeStyle: 'short' })}</div>
                  {selectedAnn.created_by_name && <div style={{ fontSize: '13px', color: '#9CA3AF', marginBottom: 20 }}>Posted by {selectedAnn.created_by_name}</div>}
      ...
      {deleteConfirmId && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ background: '#fff', borderRadius: 12, padding: 28, width: 400, maxWidth: '90vw', boxShadow: '0 8px 32px rgba(0,0,0,0.15)' }}>
            ...
            <p style={{ fontSize: '0.875rem', color: '#6B7280', margin: '0 0 20px', lineHeight: 1.55 }}>Are you sure you want to delete this announcement? This cannot be undone.</p>
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <button onClick={() => handleDelete(deleteConfirmId)} disabled={deleting} style={{ flex: 1, padding: '9px', background: '#DC2626', border: 'none', borderRadius: 8, fontWeight: 600, fontSize: '0.9rem', color: '#fff', cursor: deleting ? 'not-allowed' : 'pointer', opacity: deleting ? 0.65 : 1 }}>{deleting ? 'Deleting…' : 'Delete'}</button>
            </div>
          </div>
        </div>
      )}
          ...
          <div style={{ background: '#fff', borderRadius: 12, padding: 28, width: 480, maxWidth: '90vw', boxShadow: '0 8px 32px rgba(0,0,0,0.15)' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <div>
                ...
                <textarea value={editContent} onChange={e => setEditContent(e.target.value)} rows={5} style={{ width: '100%', padding: '9px 12px', border: '1px solid #E5E7EB', borderRadius: 7, fontSize: '0.875rem', outline: 'none', resize: 'vertical', boxSizing: 'border-box' }} />
              </div>
              ...
              <div style={{ display: 'flex', gap: 10 }}>
                ...
                <button onClick={handleSaveEdit} disabled={saving || !editTitle.trim() || !editContent.trim()} style={{ flex: 1, padding: '9px', background: ACCENT, border: 'none', borderRadius: 8, fontWeight: 600, fontSize: '0.9rem', color: '#fff', cursor: saving || !editTitle.trim() || !editContent.trim() ? 'not-allowed' : 'pointer', opacity: saving || !editTitle.trim() || !editContent.trim() ? 0.6 : 1 }}>{saving ? 'Saving…' : 'Save Changes'}</button>
              </div>
            </div>
          </div>
        </div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ background: '#fff', borderRadius: 12, padding: 28, width: 480, maxWidth: '90vw', boxShadow: '0 8px 32px rgba(0,0,0,0.15)' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <button onClick={handlePost} disabled={posting || !annTitle.trim() || !annContent.trim()} style={{ padding: '10px 0', background: ACCENT, color: '#fff', border: 'none', borderRadius: 8, fontWeight: 700, fontSize: '0.9375rem', cursor: 'pointer', opacity: posting || !annTitle.trim() || !annContent.trim() ? 0.6 : 1 }}>
