...
export default function ManagerAnnouncementsPage() {
  ...
  useEffect(() => {
    ...
    fetchAnnouncements()
    fetchUnreadMsgCount()
  }, [internalUserId, companyId, userDeptId, fetchAnnouncements, fetchUnreadMsgCount])

  useEffect(() => {
    if (!companyId) return
    const channel = supabase
      .channel('manager-announcements-page')
      .on('postgres_changes', {
  ...
  async function handlePost() {
    if (!internalUserId || !companyId || !userDeptId) return
    setPosting(true)
    try {
  ...
  const deptName = userDeptId ? (departments.find(d => d.id === userDeptId)?.name ?? 'Your Department') : 'Your Department'
                ...
                return (
                  ...
                  >
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      ...
                      <span style={{ fontSize: '0.7rem', background: ann.department_id ? ACCENT_LIGHT : '#F3F4F6', color: ann.department_id ? ACCENT : '#6B7280', padding: '1px 6px', borderRadius: 4 }}>
                ...
                <div style={{ background: '#FFFFFF', borderRadius: 12, padding: 28, boxShadow: '0 1px 4px rgba(0,0,0,0.06)', maxWidth: 720 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0, marginLeft: 16 }}>
                      <span style={{ fontSize: '0.75rem', background: selectedAnn.department_id ? ACCENT_LIGHT : '#F3F4F6', color: selectedAnn.department_id ? ACCENT : '#6B7280', padding: '3px 10px', borderRadius: 5, fontWeight: 600 }}>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ background: '#fff', borderRadius: 12, padding: 28, width: 480, maxWidth: '90vw', boxShadow: '0 8px 32px rgba(0,0,0,0.15)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
              <h3 style={{ fontWeight: 700, fontSize: '1rem', color: '#111827', margin: 0 }}>New Department Announcement</h3>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <div style={{ padding: '8px 12px', background: ACCENT_LIGHT, borderRadius: 7, fontSize: '0.8125rem', color: ACCENT, fontWeight: 500 }}>
                This will be posted to: <strong>{deptName}</strong>
              </div>
              <button
                onClick={handlePost}
