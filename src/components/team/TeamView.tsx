...
export type TeamViewPermissions = {
  canManageDepartments: boolean      // UC24-26 create/edit/delete department, incl. the department CSV import tab (UC33)
  canChangeMemberDepartment: boolean // UC31
  canInviteMembers: boolean          // UC27 send direct invitation (O/P only)
  ...
  const handleChangeDept = async () => {
    ...
    try {
      const res = await fetch('/api/user/update-department', {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ user_id: changeDeptModal.member.id, department_id: changeDeptSelectedId, requester_user_id: internalUserId }),
      })
      const data = await res.json()
      if (!data.success) throw new Error(data.message)
  ...
  const handleProfileDepartmentChange = async (departmentId: string) => {
    ...
    try {
      const res = await fetch('/api/user/update-department', {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ user_id: profileMember.id, department_id: departmentId, company_id: companyId, requester_user_id: internalUserId }),
      })
      const data = await res.json()
    ...
    try {
      ...
      await Promise.all([
        ...
        fetch('/api/user/update-department', {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ user_id: member.id, department_id: editHomeDeptId, requester_user_id: internalUserId }),
        }),
    ...
    try {
      if (departmentModal === 'add' && departmentModalTab === 'import') {
        const res = await fetch('/api/import/departments', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ company_id: companyId, departments: departmentImportRows, requester_user_id: internalUserId }),
        })
        const data = await res.json()
        ...
        const res = await fetch(isEdit ? '/api/company/update-department' : '/api/company/create-department', {
          ...
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(isEdit ? { department_id: activeDepartment?.id, name, color: departmentColorInput, requester_user_id: internalUserId } : { company_id: companyId, name, color: departmentColorInput, requester_user_id: internalUserId }),
        })
        const data = await res.json()
  ...
  const handleDeleteDepartment = async () => {
    ...
    try {
      const res = await fetch('/api/company/delete-department', {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ department_id: activeDepartment.id, requester_user_id: internalUserId }),
      })
      const data = await res.json()
        ...
        <ModalOverlay onClose={() => setProfileMember(null)} maxWidth="420px">
          <ModalBox>
            ...
            <div style={{ padding: '0 24px 20px', display: 'flex', flexDirection: 'column' }}>
              ...
              {(profileMember.role === 'Manager' || profileMember.role === 'Employee') && (
                <div style={{ padding: '14px 0', borderBottom: '1px solid #F3F4F6' }}>
                  ...
                  {permissions.canChangeMemberDepartment && canRemove(profileMember) && companyDepartments.length > 0 ? (
                    <div style={{ display: 'grid', gap: 8 }}>
                      <DropdownField
            ...
            <div style={{ padding: '0 24px 16px', display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
              ...
              <button
                onClick={() => { setProfileMember(null); router.push(`${basePath}/communication?tab=messages&partner_id=${profileMember.id}`) }}
        ...
        <ModalOverlay onClose={closeModal}>
          <ModalBox>
            ...
            <div style={{ padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 14, minHeight: 240 }}>
              ...
              {inviteTab === 'import' && permissions.canInviteMembersCsv ? (
                <>
                  {/* Sample CSV preview */}
                  <div>
                    <p style={{ margin: '0 0 8px', fontSize: '0.8125rem', fontWeight: 600, color: '#374151' }}>Sample CSV format</p>
