...
export default function ShiftsView({ sidebar, basePath, canManageShifts = true, scopeToManagerDepartments = false, scopeToEmployeeSelf = false }: {
  ...
  const fetchAssignmentData = useCallback(async (cid: string, silent = false) => {
    ...
    try {
      ...
      if (deptData.success) {
        let allDepartments: Department[] = deptData.departments ?? []
        if (scopeToManagerDepartments && internalUserId) {
          try {
            const scopeRes = await fetch(`/api/manager/departments?manager_id=${internalUserId}&company_id=${cid}`)
            const scopeData = await scopeRes.json()
            if (scopeData.success) {
              const ids = new Set((scopeData.departments as { department_id: string }[]).map(x => x.department_id))
              allDepartments = allDepartments.filter(d => ids.has(d.id))
            }
          } catch {}
        }
        const nextDepartments: Department[] = allDepartments.sort((a: Department, b: Department) => a.name.localeCompare(b.name))
    ...
    // internalUserId in deps: under manager scoping the department filter can only apply once
    // the internal user id has resolved, so the data refetches (scoped) at that point.
  ...
  const fetchCalWeek = useCallback(async (cid: string, anchorDate: string) => {
    ...
    try {
      ...
      const res = await fetch(`/api/shift?company_id=${cid}&date_from=${fmt(mon)}&date_to=${fmt(sun)}${viewerParams}`)
      const data = await res.json()
