-- 'accepted' must mean "this call won the claim" only — a repeat call on an already-accepted
-- invitation now returns 'already_accepted' so the service never double-runs the promotion /
-- shift-creation side effects on a double click.
