...
**Per-role accent color (confirmed 2026-07-20):** each role gets one accent color, applied consistently everywhere the shared UI uses "the accent color" (sidebar active nav item border/text, sidebar logo mark, drag-reorder highlight, and any other component that keys an accent off `role`) — never a full alternate theme (background, borders, typography, layout all stay identical across roles). Confirmed accents: Owner `#F97316` (orange), Partner `#18181B` (black), Manager `#3B82F6` (blue). Employee/Casual Worker/Marketing Admin/User Admin have not been assigned one yet — default to Owner's orange until specified. Implement as a single `role → color` lookup consumed wherever the shared component needs "the accent," not as scattered per-role ternaries.
